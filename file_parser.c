 ...
 * */
//U slucaju dinamicke alokacije, kreira se pointer na strukturu tipa command_buffer_t
...
static void process_command(char* line,int size){
...
#else
    //u suprotnom je globalna promjenjiva vec definisana 
#endif
    //nadji :
    command_t l_command;
    ...
    //tacka zarez pronadjeni, sada je neophodno alocirati memoriju za cuvanje linije
#ifndef DYNAMIC_ALLOCATION
    char command[10];
    memcpy(&command,line,break_location);
#else
    char* command=calloc(break_location,sizeof(char));
    memcpy(command,line,break_location);
#endif
    l_command.type=0;
    int command_idx=-1;
    ...
    memcpy(&p_command_buffer->commands[cmd_id],&l_command,sizeof(command_t));
    cmd_id++;
